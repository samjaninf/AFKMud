...
void write_class_file( int cl )
{
    ...
    for( int x = 0; x <= MAX_LEVEL; ++x )
        fprintf( fpout, "Title\n%s~\n%s~\n%s~\n%s~\n", title_table[cl][x][SEX_NEUTRAL], title_table[cl][x][SEX_MALE], title_table[cl][x][SEX_FEMALE],
                 title_table[cl][x][SEX_HERMAPHRODYTE] );
...
CMDF( do_showclass )
{
    ...
    if( !arg2.empty(  ) )
    {
        ...
        for( x = low; x <= hi; ++x )
        {
            ...
            ch->pagerf( "&wNeutral: &W%-20s &wMale: &W%-20s &wFemale: &W%-20s &wHermaphrodite: &W%s\r\n", title_table[cl][x][SEX_NEUTRAL], title_table[cl][x][SEX_MALE],
                        title_table[cl][x][SEX_FEMALE], title_table[cl][x][SEX_HERMAPHRODYTE] );
            cnt = 0;
