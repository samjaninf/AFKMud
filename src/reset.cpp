    ...
    switch ( pReset->command )
    {
        ...
        case 'Z':
            ...
            if( pReset->arg8 != -1 && pReset->arg9 != -1 && pReset->arg10 != -1 )
                snprintf( buf, MSL, "%2d) (RT object) %s (%d%%) -> Overland: %s %d %d\r\n", num, objname, pReset->arg11, map_names[pReset->arg8], pReset->arg9,
                          pReset->arg10 );
            else
                ...
                switch ( tReset->command )
                {
                    ...
                    case 'T':
                        snprintf( buf + strlen( buf ), MSL - strlen( buf ), "%2d) (trap) %d %d %d %d (%s) (%d%%) -> %s (%d)\r\n",
                                  num, tReset->arg1, tReset->arg2, tReset->arg3, tReset->arg4, flag_string( tReset->arg1, trap_flags ), tReset->arg5, objname,
                                  obj ? obj->vnum : 0 );
                        break;
