...
void check_switch( char_data * ch )
{
    ...
    for( char x = 0; x < 126; ++x )
    {
        ...
        for( icmd = cmd_list.begin(  ); icmd != cmd_list.end(  ); ++icmd )
        {
            ...
            if( !ch->isnpc(  ) && !ch->pcdata->bestowments.empty(  ) && hasname( ch->pcdata->bestowments, cmd->name )
                && cmd->level <= ch->get_trust(  ) + sysdata->bestow_dif )
                return;
        }
    }
...
void interpret( char_data * ch, string argument )
{
    ...
    if( !found )
    {
        ...
        if( !local_channel_hook( ch, command, argument )
            ...
            && !mprog_command_trigger( ch, origarg ) && !oprog_command_trigger( ch, origarg ) && !check_alias( ch, command, argument )
    ...
    if( tmptime > 3000000 )
    {
        log_printf_plus( LOG_NORMAL, ch->level, "[*****] LAG: %s: %s %s (R:%d S:%ld.%06ld)", ch->name,
                         cmd->name.c_str(  ), ( cmd->log == LOG_NEVER ? "XXX" : argument.c_str(  ) ), ch->in_room ? ch->in_room->vnum : 0, time_used.tv_sec,
                         time_used.tv_usec );
    }
