...
void fwrite_obj( char_data * ch, list < obj_data * >source, clan_data * clan, FILE * fp, int iNest, bool hotboot )
{
    ...
    for( iobj = source.begin(  ); iobj != source.end(  ); ++iobj )
    {
        ...
        if( !hotboot )
        {
            if( ( obj->item_type == ITEM_KEY && !obj->extra_flags.test( ITEM_CLANOBJECT ) ) || obj->extracted(  ) || obj->extra_flags.test( ITEM_PROTOTYPE )
                || obj->ego == -1 )
                return;
        }
            ...
            if( af->type < 0 || af->type >= num_skills )
            {
                fprintf( fp, "Affect       %d %d %d %d %d\n",
                         ...
                         ( ( af->location == APPLY_WEAPONSPELL
                             ...
                             || af->location == APPLY_RECURRINGSPELL )
                           && IS_VALID_SN( af->modifier ) ) ? skill_table[af->modifier]->slot : af->modifier, af->location, af->bit );
            }
            else
